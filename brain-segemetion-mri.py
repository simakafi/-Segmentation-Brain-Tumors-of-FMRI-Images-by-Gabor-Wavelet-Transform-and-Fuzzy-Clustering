...
def process():
    ...
    b=numpy.copy(blur)
    cv2.imwrite('color.tif', b)
    
  
    plt.subplot(121),plt.imshow(img),plt.title('master')
    plt.xticks([]), plt.yticks([])
    plt.subplot(122),plt.imshow(blur),plt.title('blurely')
    plt.xticks([]), plt.yticks([])
    plt.show()
...
def FCM():
    class FCM():
            ...
            def segmentImage(self,image_count):
                    ...
                    cv2.imwrite('output/' + str(image_count) + '.jpg' , self.Y)
                    image_count += 1
...
def ABC():
    ...
    def init(index):
        for j in range(0,D):
            r= round(random.uniform(0.0,1.0),6)
...
if __name__ == '__main__':
    ...
    w = Label(top, fg='yellow',bg='black' ,text="brain tumur detection", font=("Helvetica", 24))
    w.pack(padx=20,pady=20)
    C=ttk.Button(top, text='File Open',  style='yello', command = openFile)
